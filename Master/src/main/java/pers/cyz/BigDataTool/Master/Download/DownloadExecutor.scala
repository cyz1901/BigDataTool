...
class DownloadExecutor {
  ...
  def getBlockSize(url: URL, threadNum: Int) = {
    Try(url.openConnection().getHeaderField("Content-Length").toLong) match {
      case Success(value) => {
  ...
  def downloadExecute(array: Array[URL]) = {
...
object ExecutorTest {
  def main(args: Array[String]): Unit = {
    ...
    val array = Array(new URL("https://archive.apache.org/dist/accumulo/1.10.0/accumulo-1.10.0-bin.tar.gz.asc"),
      new URL("https://archive.apache.org/dist/accumulo/1.10.0/accumulo-1.10.0-bin.tar.gz.sha512"))
